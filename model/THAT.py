        ...
        for i, filter_size in enumerate(self.filter_sizes):
            ...
            self.__setattr__(enc_attr_name,
                             torch.nn.Conv1d(in_channels= args.input_channel,
                                       ...
                                       kernel_size=filter_size).to('cpu')
                             )
...
def main():
    args = THAT_args()
    args.input_channel = 9 #acc gyro mag
    args.input_size  = 256 #window size of the data
    args.hheads = 3 
    args.num_labels = 27
    args.SENSOR_AXIS = 1 #just set it to 1 to avoid bugs
    #model = HARTrans(args).cuda()
    #x = torch.zeros((2, 256, 9)).cuda()
    model = HARTrans(args)
    x = torch.zeros((2, 256, 9))
    out = model(x)
    print(out.size())
